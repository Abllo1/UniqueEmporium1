...
// Define the interface for a banner message, matching the DB schema
...
const messageTypes = [
  { value: "delivery_info", label: "Delivery Information" },
...
const commonIconNames = [
  ...
  { value: "Gift", label: "Gift (Promotion)" },
  { value: "BellRing", label: "Bell (Alert)" },
  { value: "Sparkles", label: "Sparkles (New Arrivals)" },
  { value: "CalendarDays", label: "Calendar (Dates)" },
  { value: "Info", label: "Info (General)" },
  { value: "Tag", label: "Tag (Offer)" },
...
const DeliveryBannerManagement = () => {
  const [bannerMessages, setBannerMessages] = useState<DeliveryBannerMessage[]>([]);
  const [searchTerm, setSearchTerm] = useState("");
  const [filterStatus, setFilterStatus] = useState("all"); // 'all', 'active', 'inactive'
  const [filterType, setFilterType] = useState("all"); // 'all', 'delivery_info', 'promotion', etc.
  const [currentPage, setCurrentPage] = useState(1);
  const messagesPerPage = 10;

  const [isAddModalOpen, setIsAddModalOpen] = useState(false);
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);
  const [editingMessage, setEditingMessage] = useState<DeliveryBannerMessage | null>(null);
  ...
  const filteredMessages = useMemo(() => {
    ...
    if (filterType !== "all") {
      filtered = filtered.filter(
        (message) => message.message_type === filterType
      );
    }

    return filtered;
  }, [bannerMessages, searchTerm, filterStatus, filterType]);
  ...
  const handleEditMessageClick = (message: DeliveryBannerMessage) => {
  ...
  const renderIcon = (iconName: string | null, className?: string) => {
    if (!iconName || !isLucideIconKey(iconName)) return null;
    ...
    >
      <div className="space-y-2">
        ...
        <motion.p className="text-base md:text-lg text-muted-foreground" variants={fadeInUp}>
          Manage the dynamic content displayed in the delivery banner across the site.
        </motion.p>
      </div>
                ...
                <TableBody>
                  <AnimatePresence>
                    {currentMessages.map((message) => (
                      ...
                      >
                        <TableCell className="font-medium max-w-[200px] truncate flex items-center gap-2">
                          {renderIcon(message.icon_name, message.text_color || "text-foreground")}
                          <span className={cn(message.text_color)} style={{ backgroundColor: message.background_color }}>
                            {message.content}
                          </span>
                        </TableCell>
                        ...
                        <TableCell className="text-xs">
                          {message.start_date ? format(new Date(message.start_date), "MMM dd, yyyy") : "N/A"} -{" "}
                          {message.end_date ? format(new Date(message.end_date), "MMM dd, yyyy") : "N/A"}
                        </TableCell>
                        <TableCell className="text-xs max-w-[150px] truncate">
                          {message.link_url ? (
                            <a href={message.link_url} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">
      ...
      <Dialog open={isAddModalOpen || isEditModalOpen} onOpenChange={isAddModalOpen ? setIsAddModalOpen : setIsEditModalOpen}>
        <DialogContent className="sm:max-w-[550px] p-6 rounded-xl shadow-lg bg-card/80 backdrop-blur-md border border-border/50 overflow-y-auto max-h-[90vh]">
          <DialogHeader>
            ...
            <DialogDescription>
              {editingMessage ? "Update the details for this banner message." : "Create a new message to display in the delivery banner."}
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleSubmit(handleAddOrUpdateMessage)} className="space-y-6 py-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                placeholder="e.g., Next Delivery Days: Tuesday, Thursday, Saturday"
                rows={2}
              />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="background_color">Background Color (Tailwind Class, Optional)</Label>
                <Input
                  id="background_color"
              ...
              <div className="space-y-2">
                <Label htmlFor="text_color">Text Color (Tailwind Class, Optional)</Label>
                <Input
                  id="text_color"
                  {...register("text_color")}
            ...
            <div className="space-y-2">
              <Label htmlFor="icon_name">Icon Name (Lucide React, Optional)</Label>
              <Select onValueChange={(value) => setValue("icon_name", value)} value={currentIconName || ""}>
                <SelectTrigger>
                  <SelectValue placeholder="Select an icon (e.g., Truck)" />
                </SelectTrigger>
                <SelectContent className="max-h-[200px]">
                  <SelectItem value="">None</SelectItem>
                  {commonIconNames.map(icon => (
                    <SelectItem key={icon.value} value={icon.value} className="flex items-center gap-2">
                      {renderIcon(icon.value, "mr-2")} {icon.label}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
              {currentIconName && renderIcon(currentIconName, "mt-2 h-6 w-6 text-primary")}
            </div>
